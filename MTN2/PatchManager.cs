...
namespace MTN2
{
    public class PatchManager
    {
        ...
        public void Initialize(IModHelper helper, IMonitor monitor) {
            ...
            ImpInitialize(helper, "Farm", "", new ConstructorFarmPatch(farmManager), typeof(ConstructorFarmPatch));
            ...
            ImpInitialize(helper, "Locations.FarmHouse", "", new ConstructorFarmHousePatch(farmManager), typeof(ConstructorFarmHousePatch));
        ...
        private void ImpInitialize(IModHelper helper, string targetClass, string routine, object obj, Type type) {
            bool prefix = false;
            bool transpiler = false;
            bool postfix = false;

            if (helper.Reflection.GetMethod(obj, "Prefix", false) != null) {
                prefix = true;
            }
            if (helper.Reflection.GetMethod(obj, "Transpiler", false) != null) {
                transpiler = true;
            }
            if (helper.Reflection.GetMethod(obj, "Postfix", false) != null) {
                postfix = true;
            }

            patches.Add(new Patch(helper, targetClass, routine, (routine == "") ? PatchType.Constructor : PatchType.Method, obj));

            patches[patches.Count - 1].Initialize(prefix, postfix, transpiler, type);
        }
